...
export function buildRefusalResponse(message) {
  ...
  // Standard refusal: neither source has anything.
  let shouldRefuse = !dashboard.hasEvidence && !sweets.hasEvidence;

  // Cross-source refusal: the user named a market (e.g., 韓國旅客喜歡哪些
  // 甜點店?) but the dashboard has no relevant rows for that market.  The
  // sweets page has no market dimension at all, so we can't answer a
  // market-specific question from it either.  Refuse instead of letting
  // the model improvise.
  if (!shouldRefuse && parsed.markets.length && !dashboard.hasEvidence) {
    shouldRefuse = true;
  }
  ...
  return {
    answer,
    display: {
      context: "大阪海外市場調査ダッシュボード / Osaka-Info 甜點專題",
      title: "該当データなし",
      level: "—",
      sectionTitle: "該当なし",
      summary: answer,
      kpis: [],
      insights: [],
      source: "大阪海外市場調査ダッシュボード + Osaka-Info 甜點專題",
...
export function buildChatContext(message) {
  ...
  const combinedStatus =
    dashboard.hasEvidence || sweets.hasEvidence ? "hasEvidence=true" : "hasEvidence=false";

  return [
    "SOURCE CONTEXT",
    "You have TWO allowed sources and MUST NOT use any other source:",
    "  (A) DASHBOARD EVIDENCE — 大阪海外市場調查_dashboard.xlsx survey numbers (markets,",
    "      items, percentages, ratings). Use these for any quantitative claim.",
    "  (B) SWEETS PAGE EVIDENCE — the single curated article at",
    "      https://osaka-info.jp/zh-Hant-TW/special/features-sweets-1/ . Use these",
    "      for sweets / café / dessert shop recommendations, addresses, hours.",
...
export function buildAnswerDisplay(message, answer) {
  ...
  const dashboardSource = sheetMeta
    ? `大阪海外市場調査ダッシュボード / ${sheetMeta.sheet_id}`
    : null;
  const sweetsSource = sweets.hasEvidence
    ? `Osaka-Info 甜點專題 / ${sweetsSourceMeta().page_title}`
    : null;
  const source =
    dashboardSource && sweetsSource
      ? `${dashboardSource} ＋ ${sweetsSource}`
      : dashboardSource || sweetsSource || buildSourceLabel(record);
  const sourceUrl = sweets.hasEvidence && !sheetMeta
    ? sweetsSourceMeta().source_url
    : "https://www.datahub.osaka-info.jp/data_03/";

  const statements = displayStatements(answer);
  const summary = statements[0] || "提供された大阪海外市場調査ダッシュボードと Osaka-Info 甜點專題に基づく回答です。";
  ...
  const sectionTitle =
    sheetMeta?.sheet_title ||
    (sweets.hasEvidence ? "Instagram映照甜點11選" : buildStageTitle(record));
...
export function buildInstructions() {
  return [
    ...
    "ABSOLUTE RULE 1: You have exactly TWO allowed sources — DASHBOARD EVIDENCE (survey numbers) and SWEETS PAGE EVIDENCE (the curated Osaka sweets feature article). Every factual statement, number, shop name, address, hour, and phone number MUST come verbatim from one of these two blocks. Never invent, never recall from training data, never use FAQ hints or the DataHub framing as a fact source.",
    "ABSOLUTE RULE 2: If COMBINED EVIDENCE STATUS is hasEvidence=false, your entire reply MUST be exactly the refusal sentence appropriate to the RESPONSE LANGUAGE: Chinese -> '目前無相關數據'; Japanese -> '大阪海外市場調査ダッシュボードに該当するデータがありません。'; English -> 'No relevant data available.' Do not add any extra sentence, disclaimer, or suggestion.",
    "ABSOLUTE RULE 3: When citing a dashboard number, append [sheet_id / 項目 / 市場] copied verbatim. When citing a sweets-page fact, append [Osaka-Info 甜點 #id 店舖名] copied verbatim from the evidence.",
    "ABSOLUTE RULE 4: Do not change units. Dashboard 百分比 cells are already shown as XX.X%. 評分 cells are 0-10 scale. Do not invent or compute percentages by hand.",
    "ABSOLUTE RULE 5: When recommending sweets, only choose items that appear in the SWEETS PAGE EVIDENCE block. If the user asks about a market preference for sweets (e.g. 'which market loves desserts'), neither source has that data — refuse per rule 2 unless the dashboard evidence also has a relevant row.",
    "Language: answer in the RESPONSE LANGUAGE specified in the user input. Default is Japanese. Translate the evidence labels naturally; keep the original label inside the [citation] tag.",
    "Style: concise, business-oriented. For sweets recommendations, list at most 3-5 items with shop, area, and hours. For dashboard data, ranked bullets when comparing markets."
  ].join(" ");
}
