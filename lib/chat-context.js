...
import { buildKbEvidence, kbSourceMeta } from "./kb-context.js";
...
export function buildRefusalResponse(message) {
  ...
  const kb = buildKbEvidence(message);
  const parsed = parseDashboardQuestion(message);

  // Standard refusal: none of the 6 sources has anything.
  let shouldRefuse =
  ...
  // for the asked topic.  Sweets/JNTO/LGBTQ/KB have no per-market
  ...
  return {
    ...
    display: {
      ...
      kpis: [],
      insights: [],
      source: "大阪海外市場調査 + 甜點專題 + JNTO + KIX 2024 + LGBTQ+ 調査 + 大阪觀光AI知識庫（いずれも該当なし）",
...
export function buildChatContext(message) {
  ...
  return [
    "SOURCE CONTEXT",
    "You have SIX allowed sources and MUST NOT use any other source:",
    ...
    "      Use for: specific Instagram-worthy sweets shops only.",
    "  (C) JNTO MACRO EVIDENCE — Japan-wide visitor stats (訪日外客数, 滞在日数, 旅行形態).",
    "      Use for: macro Japan-wide statistics.",
    ...
    "      Use ONLY when the user explicitly asks about LGBTQ+ topics.",
    "  (F) OSAKA KB EVIDENCE — 大阪觀光AI機器人知識庫 (22 chapters: districts, cuisine,",
    "      attractions, itineraries, restaurants, shopping, tickets, transport, FAQ, food",
    "      culture, seasonal events, services, experiences, pop culture, sports, day trips).",
    "      Use as the default knowledge source for general Osaka questions (history of",
    "      okonomiyaki / ramen, district intros, how to get from KIX to Namba, ticket",
    "      prices for Enjoy Eco Card, sakura spots, etc.).",
...
export function buildAnswerDisplay(message, answer) {
  ...
  if (kb.hasEvidence) sourceLabels.push(`大阪觀光AI知識庫`);
  ...
  if (sweets.hasEvidence && !sheetMeta && !kix.hasEvidence && !kb.hasEvidence) {
    sourceUrl = sweetsSourceMeta().source_url;
  } else if (kb.hasEvidence && !sheetMeta && !kix.hasEvidence) {
    sourceUrl = "https://osaka-info.jp/zh-Hant-TW/";
  }
  ...
  const titleSource =
    ...
    (kix.hasEvidence && kix.markets[0]?.name_ja
      ...
      : null) ||
    (kb.hasEvidence ? "大阪觀光AI機器人知識庫" : null) ||
  ...
  else if (kb.hasEvidence) level = "KB";
...
export function buildInstructions() {
  return [
    ...
    "ABSOLUTE RULE 1: You have exactly SIX allowed sources — (A) DASHBOARD (大阪海外市場調查), (B) SWEETS PAGE (Osaka-Info 甜點專題), (C) JNTO MACRO (Japan-wide stats), (D) KIX 2024 (関空出口調査), (E) LGBTQ+ SURVEY (Eagle Osaka 2025), (F) OSAKA KB (22-chapter Osaka tourism knowledge base). Every factual statement, number, shop name, address, hour, phone, and historical/cultural claim MUST come verbatim from one of these six blocks. Never invent, never recall from training data.",
    ...
    "  - KB:        [大阪KB / 第N章 章節名]",
    "ABSOLUTE RULE 4: Do not change units. Dashboard 百分比 already as XX.X%. 評分 cells 0-10. Do not invent percentages.",
    "ABSOLUTE RULE 5: Source selection — pick the source whose evidence block has matching content. Specifically: sweets for Instagram-worthy dessert shops; KIX for Osaka KPIs and per-market profile; dashboard for behavioural %; JNTO for Japan-wide macro; LGBTQ+ ONLY when user explicitly asks about LGBTQ+; KB for general Osaka knowledge (districts, cuisine history, attraction intros, transport, tickets, seasonal events, day trips).",
    "ABSOLUTE RULE 6: Do NOT mix LGBTQ+ survey numbers into a general Osaka answer (niche Eagle Osaka subsample, not representative). Use LGBTQ+ data only on LGBTQ+ topics.",
    "ABSOLUTE RULE 7: When KB and KIX both have content for an attraction (e.g., 通天閣), KB describes the place itself while KIX describes which markets visit it; combine them when both are relevant.",
    "ABSOLUTE RULE 8: The KB source text is written in Traditional Chinese (zh-Hant). When the RESPONSE LANGUAGE is Japanese (default), translate the KB facts you cite into natural Japanese. Keep the Chinese place names that have a Japanese-equivalent kanji form (e.g., 心齋橋→心斎橋, 灣區→湾岸エリア, 黑門市場→黒門市場); use standard kanji/kana spellings in the answer. Never copy raw zh-Hant prose or zh-Hant punctuation (，。、) into a Japanese answer.",
    "Language: answer in the RESPONSE LANGUAGE specified in the user input. Default is Japanese.",
    "Style: concise, business-oriented. For sweets / restaurant / shop recommendations: list EVERY shop in the evidence that fits the user's area or theme — at least 5 and up to 8 — each as a bullet with 店名・エリア・営業時間 (and a one-line description if space allows). Do not arbitrarily trim to 3. For market comparisons: ranked bullets."
  ].join(" ");
}
