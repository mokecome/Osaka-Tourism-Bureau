...
// Exported so KIX (kix-context.js) can reuse the same matching logic
// without redefining 12-market vocabulary.
export const MARKET_ALIASES = new Map([
...
function aliasMatches(alias, normalized) {
  ...
  if (!a) return false;
  // Short Latin codes (us, hk, tw, cn, kr, my, id, sg, th, vn, ph, ca)
  // would otherwise substring-match "usj", "candy", "video", "trust", etc.
  // Require word-boundary alignment for any alias that is ≤3 chars and
  // purely Latin/digits.
...
function scoreCell(cell, parsed) {
  ...
  // Topic relevance: how strongly does the question vocabulary touch THIS
  // particular cell (its item or sheet title)?  This is what protects us
  // from off-topic questions that happen to mention a market.
  let topicHits = 0;
  for (const token of parsed.tokens) {
    if (!token) continue;
    if (itemText.includes(token)) topicHits += 8;
    if (sheetText.includes(token)) topicHits += 2;
  }
  const sheetMatched = parsed.sheets.includes(cell.sheet_id);
  const hasTopicRelevance = topicHits > 0 || sheetMatched;

  // Wrong-market cells are dropped hard.  Right-market cells with zero
  // topic relevance are ALSO dropped, otherwise asking "韓國旅客喜歡哪些
  // 甜點店?" would surface unrelated rows like 男女比 just because 韓國
  // matched.
  let marketScore = 0;
  if (parsed.markets.length) {
    if (!parsed.markets.includes(cell.market)) return -100;
    if (!hasTopicRelevance) return -100;
    marketScore = 40;
  }

  let sheetScore = 0;
  if (parsed.sheets.length) {
    if (sheetMatched) sheetScore = 18;
    else sheetScore = -4;
  }

  return marketScore + sheetScore + topicHits;
}
