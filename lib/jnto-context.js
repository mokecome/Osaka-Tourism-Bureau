...
const JNTO_TOPIC_KEYWORDS = [
  ...
  "都道府県", "都道府縣", "訪問率", "访问率", "visit rate",
  "訪問率ランキング", "訪問先ランキング", "訪れた", "最も多く訪れた",
  "トップ8", "訪問先",
  // Market performance comparison (per-market series surface via gating)
  "パフォーマンス", "市場の違い", "市場別の違い",
  // Covid recovery framing
  "コロナ", "コロナ禍", "回復", "コロナからの回復",
  // Overnight stays (macro)
  "延べ宿泊", "宿泊者数", "宿泊者數", "人泊", "overnight stays",
  // Expectations framework (JNTO-specific phrasing)
  "次回やりたい", "次回やりたいこと", "やりたいこと", "次回意向", "リピート意向",
  "訪日前に期待", "訪日中に実施", "訪日旅行で期待", "体験項目",
  // JNTO source brand
...
function scoreFact(fact, parsed) {
  ...
  const tags = (fact.tags || []).map(normalizeText);
  ...
  for (const token of parsed.tokens) {
    ...
    // Curated tags get the highest per-hit weight so a tagged fact
    // (e.g. expectations for 「次回やりたい」, prefecture for 「都道府県」)
    // reliably ranks above incidental topic overlaps.
